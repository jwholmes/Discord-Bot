...
async function process(message, data) {
  ...
  if (reactions.has(emojiAnswer)) {
    ...
    console.log(score);
    topicsToRevise.push(`${randomQuestion.title}`);
    console.log(topicsToRevise);
    await message.author.send(
      new Discord.MessageEmbed()
        .setColor('#F96F51')
        .setTitle(`Points: ${score * 100}`)
        .setDescription(`The answer was ${emojiAnswer}`)
    );
    if (Math.random() > 0.5) {
  ...
  async execute(message, args) {
    const subjectMessage = await message.author.send(
      new Discord.MessageEmbed().setColor("#FFD800").setTitle('QUIZ TIME!').setDescription(`**Choose your subject 📚**
        ...
        \n💻 ComSci
        \n🛠 DT
        \n🕵️‍♀️ English Lit - An Inspector Calls
        \n📚 English Language
        \n🇫🇷 French
        \n🌍 Geography
    ...
    await subjectMessage.react('💻');
    await subjectMessage.react('🛠');
    await subjectMessage.react('🕵️‍♀️');
    await subjectMessage.react('📚');
    await subjectMessage.react('🇫🇷');
    await subjectMessage.react('🌍');
    await subjectMessage.react('🇩🇪');
    ...
    } else if (subjectReaction.has('🕵️‍♀️')) {
      for (let i = 0; i < 10; i++) {
